...
local Typer, utf8, lg = {}, require("utf8"), love.graphics
...
function TXT.update(dt)
	...
	if curr_msg.paused or curr_msg.complete then 
		TXT.ind_timer=TXT.ind_timer+1 
		if TXT.ind_timer>TXT.ind_delay then 
			TXT.show_ind=not TXT.show_ind
			TXT.ind_timer=0 end
		else 
			TXT.show_ind=false 
		end

	if curr_msg:update(dt) then 
		if curr.typed_not_talked then 
			playSound(curr.talk_sound) 
		elseif not curr.talk_sound:isPlaying() then 
			local pitch = curr.pitch_values[math.random(#curr.pitch_values)]; playSound(curr.talk_sound, pitch) 
		end 
	end
end
...
function TXT.draw(func)
    ...
    love.graphics.setStencilTest()
		...
		lg.setFont(curr.font)
		lg.setColor(curr.ttl_color)
		lg.print(curr.ttl,ttlX,ttlY) -- msg ttl

		lg.setColor(0, 0, 0, 0.1)
		lg.rectangle("fill", boxX, boxY, boxW, boxH) -- Main msg box

		if curr.image ~= nil then 
			lg.push()
			lg.setColor(255, 255, 255)
			lg.draw(curr.image, imgX, imgY, 0, imgScale, imgScale)
			lg.pop() 
		end -- msg avatar

		lg.setColor(curr.msg_color)
		lg.printf(curr_msg.visible, txt_x, txt_y, boxW - imgW - (4 * curr.padding)) -- msg text
    ...
    if curr:show_opts() and curr_msg.complete then
		...
		if TXT.show_ind then 
			lg.print(curr.ind_char, boxX+boxW-(2.5*curr.padding), boxY+boxH-curr.fontHeight) 
		end -- Next msg/continue ind

    lg.pop()
end

function TXT.advance_msg() 
	local curr = TXT.dialogs:peek()
	if curr == nil then return end
	if curr.msgs:len() == 1 then 
		curr.oncomplete()
		TXT.dialogs:pop() 
		if TXT.dialogs:len() == 0 then 
			TXT.clear() 
		else 
			TXT.dialogs:peek().onstart() 
		end 
	end
	curr.msgs:pop()
	curr.onmsg(curr.msgs:len()) 
end

function TXT.prev_opt() 
	local curr = TXT.dialogs:peek()
	if curr == nil or not curr:show_opts() then return end
	curr.opt_index = curr.opt_index - 1
	if curr.opt_index < 1 then 
		curr.opt_index = #curr.options 
	end
	playSound(curr.opt_sound) 
end

function TXT.next_opt() 
	local curr = TXT.dialogs:peek()
	if curr == nil or not curr:show_opts() then return end
	curr.opt_index = curr.opt_index + 1
	if curr.opt_index > #curr.options then 
		curr.opt_index = 1 
	end
	playSound(curr.opt_sound)
end

function TXT.on_action() 
	local curr = TXT.dialogs:peek()
	if curr == nil then return end
	local curr_msg = curr.msgs:peek()

	if curr_msg.paused then 
		curr_msg:resume() 
	elseif not curr_msg.complete then 
		curr_msg:finish() 
	else 
		if curr:show_opts() then 
			curr.options[curr.opt_index][2]()
			playSound(curr.opt_sound) 
		end 
		TXT.advance_msg() 
	end 
